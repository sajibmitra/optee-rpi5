...
-  ``ARM_FW_CONFIG_LOAD_ENABLE``: Boolean option to enable the loading of
   ...
   BL2 is responsible for loading, parsing, and validating the FW_CONFIG device
   trees from the FIP. The option depends on RESET_TO_BL2.
...
- ``FVP_TRUSTED_SRAM_SIZE``: Size (in kilobytes) of the Trusted SRAM region to
  utilize when building for the FVP platform. This option defaults to 384.
